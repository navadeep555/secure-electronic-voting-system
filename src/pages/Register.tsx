...
const DISTRICTS: { [key: string]: string[] } = {
  AP: ["Anantapur", "Chittoor", "East Godavari", "Guntur", "Krishna", "Kurnool", "Prakasam", "Srikakulam", "Sri Potti Sriramulu Nellore", "Visakhapatnam", "Vizianagaram", "West Godavari", "YSR District", "Kadapa", "Tirupati", "Nandyal", "Bapatla", "Palnadu", "Konaseema", "Eluru", "Anakapalli", "Kakinada"],
  AR: ["Tawang", "West Kameng", "East Kameng", "Papum Pare", "Kurung Kumey", "Kra Daadi", "Lower Subansiri", "Upper Subansiri", "West Siang", "East Siang", "Siang", "Upper Siang"],
  ...
  TN: ["Chennai", "Coimbatore", "Madurai", "Tiruchirappalli", "Salem", "Tirunelveli", "Tiruppur", "Vellore", "Erode", "Thoothukudi", "Dindigul", "Thanjavur"],
  MH: ["Mumbai City", "Mumbai Suburban", "Thane", "Pune", "Nashik", "Nagpur", "Ahmednagar", "Solapur", "Jalgaon", "Amravati", "Kolhapur"],
  // Adding a default fallback for other states to avoid empty lists in this refactor
  UP: ["Lucknow", "Kanpur", "Varanasi", "Agra", "Meerut", "Ghaziabad", "Prayagraj", "Bareilly", "Aligarh", "Moradabad"],
  KA: ["Bengaluru Urban", "Bengaluru Rural", "Mysuru", "Hubballi-Dharwad", "Mangaluru", "Belagavi", "Davangere", "Ballari", "Vijayapura", "Shivamogga"],
};
...
export default function Register() {
  ...
  const validateStep1 = () => {
    ...
    if (!/^[a-zA-Z\s]{2,}$/.test(personalInfo.fullName)) newErrors.fullName = "Invalid name";
      ...
      reader.onload = async (e) => {
        ...
        if (field === "voterIdFile") {
          ...
          try {
            ...
            if (result.success) setDocumentUpload(prev => ({ ...prev, voterIdVerified: true }));
        ...
        <div className="min-h-screen bg-neutral-100 py-12 px-4 relative">
          ...
          <div className="container max-w-4xl relative z-10">
            ...
            <div className="text-center mb-10">
              ...
              <h1 className="text-3xl font-display font-bold text-neutral-900 mb-2">Voter Registration Portal</h1>
              <p className="text-neutral-500 max-w-2xl mx-auto">
                Official enrollment for the National Electoral Roll. Please follow the steps below to verify your identity and register your biometric data.
              </p>
            </div>
            ...
            <div className="bg-white rounded-sm shadow-xl border-t-4 border-primary-700">
                ...
                <div className="bg-neutral-50 border-b border-neutral-200 px-8 py-6">
                  <div className="flex items-center justify-between">
                    {[
                      ...
                      <div key={step.num} className="flex flex-col items-center relative z-10 group">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all mb-2 border-2 
                          ${currentStep === step.num ? 'bg-primary-700 text-white border-primary-700 scale-110 shadow-lg' :
                              currentStep > step.num ? 'bg-green-600 text-white border-green-600' : 'bg-white text-neutral-400 border-neutral-300'}`}
                        >
                          {currentStep > step.num ? <Check className="h-5 w-5" /> : step.num}
                        </div>
                        <span className={`text-xs font-bold uppercase tracking-wider ${currentStep === step.num ? 'text-primary-800' : 'text-neutral-400'}`}>
                          {step.label}
                        </span>
                      </div>
                    ))}
              ...
              {showSuccess ? (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-12 text-center">
                  ...
                  <h2 className="text-3xl font-display font-bold text-neutral-900 mb-4">Registration Complete</h2>
                  <p className="text-neutral-600 text-lg mb-8 max-w-lg mx-auto">
                    Your application has been successfully submitted to the National Electoral Rolls.
                    <br /><br />
                    <span className="font-bold text-neutral-900">Application ID: REF-{Math.floor(Math.random() * 1000000)}</span>
                  </p>
                  <Button onClick={() => window.location.href = "/"} className="bg-primary-700 hover:bg-primary-800 text-white px-8 py-4 rounded-sm font-bold uppercase tracking-wide">
                    Return to Home
                  </Button>
                </motion.div>
              ) : (
                      ...
                      <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Full Name (as per Aadhaar)</label>
                            <input type="text" value={personalInfo.fullName} onChange={e => /^[a-zA-Z\s]*$/.test(e.target.value) && setPersonalInfo({ ...personalInfo, fullName: e.target.value })} className="w-full p-3 border border-neutral-300 rounded-sm focus:ring-1 focus:ring-primary-700 bg-neutral-50" placeholder="e.g. Rahul Sharma" />
                            {errors.fullName && <p className="text-red-600 text-xs flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {errors.fullName}</p>}
                          </div>
                          <div className="space-y-2">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Date of Birth</label>
                            <input type="date" value={personalInfo.dateOfBirth} onChange={e => setPersonalInfo({ ...personalInfo, dateOfBirth: e.target.value })} className="w-full p-3 border border-neutral-300 rounded-sm focus:ring-1 focus:ring-primary-700 bg-neutral-50" />
                            {errors.dateOfBirth && <p className="text-red-600 text-xs flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {errors.dateOfBirth}</p>}
                          </div>
                          <div className="space-y-2">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Aadhaar Number</label>
                            <input type="text" value={personalInfo.aadhaarNumber} onChange={e => {
                          ...
                          <div className="space-y-2">
                            ...
                            {errors.state && <p className="text-red-600 text-xs flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {errors.state}</p>}
                          </div>
                          <div className="space-y-2">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wide">District</label>
                            <input type="text" value={personalInfo.district} onFocus={() => setIsDistrictFocused(true)} onChange={e => setPersonalInfo({ ...personalInfo, district: e.target.value })} className="w-full p-3 border border-neutral-300 rounded-sm focus:ring-1 focus:ring-primary-700 bg-neutral-50" disabled={!personalInfo.state} placeholder={personalInfo.state ? "Type to search..." : "Select State First"} />
                            {personalInfo.state && isDistrictFocused && DISTRICTS[personalInfo.state] && (
                              <div className="absolute bg-white border border-neutral-200 shadow-lg mt-1 max-h-40 overflow-y-auto w-64 z-50">
                                {DISTRICTS[personalInfo.state]?.filter(d => d.toLowerCase().includes(personalInfo.district.toLowerCase())).map(d => (
                                  <div key={d} className="p-2 hover:bg-neutral-100 cursor-pointer text-sm" onClick={() => { setPersonalInfo({ ...personalInfo, district: d }); setIsDistrictFocused(false); }}>{d}</div>
                                ))}
                              </div>
                            )}
                            {errors.district && <p className="text-red-600 text-xs flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {errors.district}</p>}
                          </div>
                        </div>
                      </motion.div>
                    )}
                    ...
                    {currentStep === 3 && (
                      <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6 text-center">
                        <div className="bg-primary-50 border-l-4 border-primary-700 p-4 text-left">
                          <h4 className="font-bold text-primary-900 text-sm mb-1">Official Facial Biometric Enrollment</h4>
                          <p className="text-primary-800 text-xs">Please remove glasses and masks. Ensure good lighting.</p>
                        </div>
                        ...
                        <div className="relative mx-auto bg-black rounded-sm overflow-hidden border-4 border-neutral-200 shadow-inner w-full max-w-lg aspect-video">
                          {!cameraActive ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-neutral-100">
                              ...
                              <Button onClick={startCamera} className="bg-primary-700 hover:bg-primary-800 text-white font-bold uppercase tracking-wide">
                            ...
                            <>
                              ...
                              <div className="absolute bottom-0 inset-x-0 bg-black/60 p-4 text-white">
                                <p className="text-lg font-bold">{detectionMessage}</p>
                                <p className="text-xs uppercase tracking-widest text-neutral-400 mt-1">Stage {currentStage}/4</p>
                              </div>
                            </>
                          )}
                        ...
                        <div className="grid grid-cols-4 gap-4 mt-6">
                          {[1, 2, 3, 4].map(num => (
                            <div key={num} className={`p-3 border rounded-sm ${biometricData[`stage${num}Captured` as keyof BiometricData] ? 'border-green-500 bg-green-50' : 'border-neutral-200 bg-neutral-50'} transition-all`}>
                              <div className="text-center">
                                <span className="text-xs font-bold uppercase text-neutral-500 mb-2 block">Angle {num}</span>
                                {biometricData[`stage${num}Captured` as keyof BiometricData] ? (
                                  <div className="flex justify-center text-green-600"><CheckCircle2 className="h-6 w-6" /></div>
                                ) : (
                        ...
                        {cameraActive && (
                          <div className="flex gap-4 justify-center mt-4">
                            <Button variant="outline" onClick={handleRescan} className="border-neutral-300 text-neutral-700 hover:bg-neutral-100">Reset Scan</Button>
                          </div>
                        )}
                      </motion.div>
                    )}
                    ...
                    {currentStep === 4 && (
                      <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                        <div className="bg-neutral-50 p-6 border border-neutral-200 rounded-sm">
                          <h3 className="font-bold text-lg text-neutral-900 mb-4 border-b border-neutral-200 pb-2">Applicant Summary</h3>
                          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6 text-sm">
                            <div>
                              <dt className="text-neutral-500 uppercase text-xs font-bold">Full Name</dt>
                              <dd className="font-semibold text-neutral-900 mt-1">{personalInfo.fullName}</dd>
                            </div>
                            <div>
                              <dt className="text-neutral-500 uppercase text-xs font-bold">Aadhaar Reference</dt>
                              <dd className="font-semibold text-neutral-900 mt-1">{personalInfo.aadhaarNumber.replace(/.(?=.{4})/g, 'X')}</dd>
                            </div>
                            <div>
                              <dt className="text-neutral-500 uppercase text-xs font-bold">Residence</dt>
                              <dd className="font-semibold text-neutral-900 mt-1">{personalInfo.district}, {INDIAN_STATES.find(s => s.code === personalInfo.state)?.name}</dd>
                            </div>
                            <div>
                              <dt className="text-neutral-500 uppercase text-xs font-bold">Verification Status</dt>
                              <dd className="font-semibold text-green-700 mt-1 flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> Verified</dd>
                            </div>
                          </dl>
                        </div>
                        <div className="flex items-center gap-2 text-xs text-neutral-500 bg-neutral-100 p-3 rounded-sm">
                          <Shield className="h-4 w-4" />
                          By submitting, you declare that the information provided is true and correct. False declaration is a punishable offense.
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
